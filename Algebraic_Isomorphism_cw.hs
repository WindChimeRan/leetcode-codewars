...
isoEither (ab, ba) (cd, dc) = (to, from)
  where
    to (Left a) = Left (ab a)
    to (Right c) = Right (cd c)
    from (Left b) = Left (ba b)
    from (Right d) = Right (dc d)
...
isoUnMaybe (ab, ba) = (uab, uba)
  where
    uab a = fromMaybe (fromJust $ ab Nothing) $ ab (Just a)                                                                                                                    
    uba b = fromMaybe (fromJust $ ba Nothing) $ ba (Just b)
...
dist = (from, to)
  where
    from (a, Left b) = Left (a, b)
    from (a, Right c) = Right (a, c)
    to (Left (a, b)) = (a, Left b)
    to (Right (a, c)) = (a, Right c)
...
two = (from, to)
  where
    from True = Nothing
    from False = Just Nothing
    to Nothing = True
    to (Just Nothing) = False
...
plusS = (from, to)
  where
    from (Left (Just a)) = Just (Left a)
    from (Left Nothing) = Nothing
    from (Right b) = Just (Right b)
    to (Just (Left a)) = Left (Just a)
    to Nothing = Left Nothing
    to (Just (Right b)) = Right b
...
multS = (from, to)
  where
    from (Just a, b) = Right (a, b)
    from (Nothing, b) = Left b
    to (Left b) = (Nothing, b)
    to (Right (a, b)) = (Just a, b)
...
powS = (from, to)
  where
    from mba = (mba Nothing, mba . Just)
    to (a, ba) = (\mb -> case mb of
                                    Nothing -> a
                                    Just b  -> ba b)
